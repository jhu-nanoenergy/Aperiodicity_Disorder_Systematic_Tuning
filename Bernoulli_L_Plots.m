...
% for plasmonic nanodisks distributed according to a Bernoulli point process parametrized by unit cell size.
%
...
NUMTRIALS = 5;
params = [3.6e-6, 4.8e-6, 6e-6, 7.2e-6];
params_s = "L = " + ["3.6", "4.8", "6.0", "7.2"] + " µm";
names = params_s;
param_names = ["3x3_n15","4x4_n20","5x5_n25","6x6_n30"];
max_upper = 4.8e-6;
pa = 4^2 * 1/max_upper^2;
a_vec = max_upper ./ params;
P_VEC = pa*a_vec.^2;
P_VEC = params * 1e6;
fnamesbase = ["Bernoulli_L/Bernoulli_" + param_names];
...
for ind2 = 1:length(fnames)
    ...
    load(fnames(ind2));
    L_vec = [3.6e-6, 4.8e-6, 6e-6, 7.2e-6];
    L = L_vec(ind2);
